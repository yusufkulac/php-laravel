...
@section('title','Çözüm Ortağı Düzenle')
...
<section class="sayfa-icerik-kapsayici px-3">
	...
	@else
		<form id="cozumOrtaklariGuncelleForm">		
			<input type="hidden" name="id" value="{{ $cozumOrtagi->id }}">
			<div class="my-3 p-2" style="height:130px; width:270px; box-shadow: 0 0 10px #777; overflow:hidden;">
				@if(is_file("uploads/resim/cozum_ortaklari/".$cozumOrtagi->logo))
                    <img src="{{ asset('uploads/resim/cozum_ortaklari/'.$cozumOrtagi->logo) }}" class="img-fluid">
                @else
			...
			<div class="form-group my-2">
				...
				<input type="text" name="ortak_adi" class="form-control" value="{{ $cozumOrtagi->ortak_adi }}">
			</div>			 		
...
$(function(){
	...
	//çözüm ortağı güncelle --------------------------------------------------------------
    $("#cozumOrtaklariGuncelleForm").submit(function(e){
        e.preventDefault();                
        ...
        $.ajax({
            type        : "post",
            url         :"{{ route('panel.cozum.ortaklari.duzenle.post') }}",
            data        : form_data,
            contentType : false,            
            processData : false,            
            dataType    :'json',
