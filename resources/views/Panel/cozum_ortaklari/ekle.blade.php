...
@section('title','Çözüm Ortağı Ekle')
...
<section class="sayfa-icerik-kapsayici px-3">
	<form id="cozumOrtakEkleForm" enctype="multipart/form-data">		
		...
		<div class="form-group my-2">
			<label for="">Çözüm Ortağı Adı : (Opsiyonel)</label>
			<input type="text" name="ortak_adi" class="form-control">
		</div>
...
$(function(){
	...
	//çözüm ortağı Ekle --------------------------------------------------------------
    $("#cozumOrtakEkleForm").submit(function(e){
        e.preventDefault();
        ...
        if( $("#logo").get(0).files.length === 0 ) {
        	Swal.fire({
                type: 'error', 
                text: 'Çözüm ortağı resim/logo ekleyiniz',
            ...
            $.ajax({
                type        : "post",
                url         :"{{ route('panel.cozum.ortaklari.ekle.post') }}",
                data        : form_data,
                contentType : false,            
                processData : false,            
                dataType    :'json',
