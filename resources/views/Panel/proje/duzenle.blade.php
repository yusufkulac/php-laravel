...
<section class="sayfa-icerik-kapsayici px-3">
	...
	@if($proje == null)
		<div class="alert alert-danger d-flex align-items-center">
			<i class="fas fa-info-circle me-2"></i> Böyle bir proje bulunamadı			
		</div>
	@else		
		<form id="projeDuzenleForm" enctype="multipart/form-data">	
			...
			<div class="tab-content p-2 border" id="pills-tabContent">
				...
				<div class="tab-pane fade show active" id="pills-bilgi" role="tabpanel" aria-labelledby="pills-bilgi-tab">
					...
					<div class="form-group my-2">
						...
						<select name="dil_id" class="form-control">				
							@foreach ($diller as $dil)
								<option value="{{ $dil->id }}" @if($proje->dil_id == $dil->id) selected @endif>
									{{ $dil->dil_adi }}
								</option>
							@endforeach
						</select>
					...
					<div class="form-group my-3">
						...
						<input type="text" name="proje_adi" class="form-control" value="{{ $proje->proje_adi }}">
					</div>

					<div class="form-group my-2">
						<label for="">Description : <small>(Opsiyonel)</small></label>
						<textarea name="description" class="form-control" rows="2">{{ $proje->description }}</textarea>
					</div>

					<div class="form-group my-2">
						<label for="">Keywords : <small>(Opsiyonel)</small></label>
						<textarea name="keywords" class="form-control" rows="2">{{ $proje->keywords }}</textarea>
					</div>

					<div class="form-group my-2">
						<label for="">İşveren :</label>
						<input type="text" name="isveren" class="form-control" value="{{ $proje->isveren }}">
					</div>

					<div class="form-group my-2">
						<label for="">Lokasyon :</label>
						<input type="text" name="lokasyon" class="form-control" value="{{ $proje->lokasyon }}">
					</div>

					<div class="form-group my-2">
						<label for="">Hizmet Alanı m<sup>2</sup> :</label>
						<input type="text" name="hizmet_alani" class="form-control" value="{{ $proje->hizmet_alani }}">
					</div>

					<div class="row form-group my-2">
						<div class="col-sm-6">
							<label for="">Başlama Tarihi :</label>
							<input type="text" name="baslama_tarihi" class="form-control" value="{{ $proje->baslama_tarihi }}">
						</div>
						<div class="col-sm-6">
							<label for="">Bitiş Tarihi :</label>
							<input type="text" name="bitis_tarihi" class="form-control" value="{{ $proje->bitis_tarihi }}">
						</div>			
					</div>
					
					<div class="form-group my-3">
						<label>Proje Özeti :</label>
						<textarea name="ozet" class="editor-small">{{ $proje->ozet }}</textarea>
					</div>

					<div class="form-group my-3">
						<label>Proje Açıklaması :</label>
						<textarea name="aciklama" class="editor">{{ $proje->aciklama }}</textarea>
					</div>

					<div class="form-group my-2">
						<label>Durumu:</label>
						<select name="durumu" class="form-control">
							<option value="devam" @if($proje->durumu == "devam") selected @endif>Devam Ediyor</option>
							<option value="tamam" @if($proje->durumu == "tamam") selected @endif>Tamamlandı</option>
							<option value="plan" @if($proje->durumu == "plan") selected @endif>Planlandı</option>
						</select>
					</div>
					...
					<div class="form-group my-3">
						...
						<select name="aktif" class="form-control">
							<option value="0" @if($proje->aktif == 0){{ 'selected' }} @endif>Pasif</option>
							<option value="1" @if($proje->aktif == 1){{ 'selected' }} @endif>Aktif</option>
						</select>
					</div>

					<div class="form-group mb-3 my-4">
						<input type="checkbox" name="anasayfa_goster" id="anasayfa" @if($proje->anasayfa_goster == 1){{ 'checked' }} @endif>
						<label for="anasayfa">Anasayfada Göster</label>                
					</div>

					<div class="alert alert-primary d-flex align-items-center">
						<i class="fas fa-info-circle me-2"></i>
						Kelime aralarına , koyunuz. Örnek: etiket1, etiket2				
					</div>
					
					<div class="form-group mt-0">
						<label>Etiketler:</label>
						<textarea name="etiket" class="form-control" rows="2">{{ $proje->etiket }}</textarea>
					</div>

				</div><!-- end bilgileri content ------------------------------------------------------------>
				...
				<div class="tab-pane fade" id="pills-resim" role="tabpanel" aria-labelledby="pills-resim-tab">
						...
						<div style="width:480px; height: 44vh;" class="">
							...
							<div class="p-2 text-center bg-white" style="height:30vh; width:100%; box-shadow: 0 0 10px #777;">
								...
								@if(is_file("uploads/resim/proje/".$proje->resim))
					                <img src="{{ asset('uploads/resim/proje/'.$proje->resim) }}" style="height:100%; width:auto">
					            @else
					...
					<div class="resim-ekle-wrap my-3 font-weight-bold" style="font-size: 16px">
						<i class="fas fa-images"></i> Projenin Diğer Resimleri
					</div> 		

						@if( count($proje->resimler) == 0 || $proje->resimler == null )
							<div class="alert alert-danger d-flex align-items-center">
								<i class="fas fa-info-circle me-2"></i> projea diğer resim eklenmemiş			
							</div>
						...
						@else
							...
							<div class="row" id="sortable">
								@foreach($proje->resimler as $resim)
										...
										<div class="col-6 col-md-4 col-lg-3 sortable-wrap sortable" data-index="{{ $resim->id }}" data-sira="{{ $resim->sira }}">
											<div class="eski-resim-wrapper">
												<div class="erw-inside">
													<img src="{{ asset('uploads/resim/proje/'.$resim->resim) }}">
												</div>
											</div>
											<div class="text-center my-2">
												<button type="button" class="btn btn-danger proje-resim-sil px-2 py-0" data-bs-toggle="tooltip" data-placement="bottom" title="Sil" data-id="{{ $resim->id }}">
				...
				<div class="tab-pane fade" id="pills-video" role="tabpanel" aria-labelledby="pills-video-tab">
					<div class="alert alert-primary d-flex align-items-center mt-5">
						<i class="fas fa-info-circle me-2"></i>
						Proje ile ilgili youtube'daki videonuzun linkini veya sitenize video dosyasını bu bölümden ekleyin
					</div>
					...
					<div class="tab-content" id="pills-tabContent">
						<div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="bilgiler">
							<textarea name="video_link" class="form-control" rows="3">{{$proje->video_link}}</textarea>
						</div>
						   ...
						   <div class="my-4">
						   		@if(is_file("uploads/video/proje/".$proje->video))
									<video width="320" height="240" controls>
										<source src="/uploads/video/proje/{{$proje->video}}" type="video/mp4">	
										<div class="alert alert-danger">
											Tarayıcınız video formatını desteklemiyor
										</div>
									</video> 
									<br>
									<button type="button" class="btn btn-danger video-sil my-2" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Sil" data-id="{{ $proje->id }}">
										<i class="far fa-trash-alt"></i>
									</button>
								@endif
						   </div>
...
$(function(){	
	...
	//proje resmini sil ----------------------------------------------------------
    $(".proje-resim-sil").on("click", function(){                
		...
		Swal.fire({
			...
			if (result.value) {
				...
				var i  = $(".proje-resim-sil").index(this);
				var id = $(".proje-resim-sil").eq(i).data("id");                

				$.ajax({
					type        : "post",
					url         : "{{ route('panel.proje.resim.sil') }}",
					data        : {id:id},
	...
	$("#projeDuzenleForm").submit(function(e){
		...
		var proje_adi = $.trim( $("[name=proje_adi]").val() );
		...
		if(proje_adi <= 0){
			Swal.fire({
				...
				$("[name=proje_adi]").addClass("border-red");
			...
			$.ajax({
				type        : "post",
				url         :"{{ route('panel.proje.duzenle.post') }}",
				data        : form_data,
				contentType : false,            
				processData : false,            
				dataType    :'json',
    ...
    $( "#sortable" ).sortable({
        ...
        update : function(event, ui){
            ...
            $.ajax({
                ...
                url      : "{{ route('panel.proje.resim.sirala') }}",
    ...
    $(".video-sil").on("click", function(){  
		...
		Swal.fire({
			...
			if (result.value) {
				...
				$.ajax({
					type        : "post",
					url         : "{{ route('panel.proje.video.sil') }}",
					data        : {id:id},
