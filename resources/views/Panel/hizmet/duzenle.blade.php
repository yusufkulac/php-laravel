...
<section class="sayfa-icerik-kapsayici px-3">
	...
	@if($hizmet == null)
		<div class="alert alert-danger d-flex align-items-center">
			...
			Böyle bir hizmet bulunamadı
		</div>
	@else					
				
		<form id="hizmetDuzenleForm" enctype="multipart/form-data">	
			...
			<div class="tab-content p-2 border" id="pills-tabContent">
				...
				<div class="tab-pane fade show active" id="pills-bilgi" role="tabpanel" aria-labelledby="pills-bilgi-tab">
					...
					<div class="form-group my-2">
						...
						<select name="kategori_id" class="form-control">				
							@foreach ($kategoriler as $kategori)
								<option value="{{ $kategori->id }}" @if($kategori->id == $hizmet->kategori_id) selected @endif>
					...
					<div class="form-group my-3">
						...
						<input type="text" name="sira" class="form-control" value="{{ $hizmet->sira }}">
					</div>

					<div class="form-group my-3">
						<label for="">Hizmet Adı :</label>
						<input type="text" name="baslik" class="form-control" value="{{ $hizmet->baslik }}">
					</div>

					<div class="form-group my-3">
						<label>Özet:</label>
						<textarea name="ozet" class="editor-small">{{ $hizmet->ozet }}</textarea>
					</div>

					<div class="form-group my-3">
						<label>Hizmet Açıklaması:</label>
						<textarea name="icerik" class="editor">{{ $hizmet->icerik }}</textarea>
					</div>
					
					<div class="form-group my-3">
						<label>Aktif / Pasif:</label>
						<select name="aktif" class="form-control">
							<option value="0" @if($hizmet->aktif == 0){{ 'selected' }} @endif>Pasif</option>
							<option value="1" @if($hizmet->aktif == 1){{ 'selected' }} @endif>Aktif</option>
						</select>
					</div>

					<div class="form-group mb-3 my-4">
						<input type="checkbox" name="anasayfa_goster" id="anasayfa" @if($hizmet->anasayfa_goster == 1){{ 'checked' }} @endif>
					...
					<div class="form-group mt-0 my-3">
						<label>Etiketler:</label>
						<textarea name="etiket" class="form-control" rows="2">{{ $hizmet->etiket }}</textarea>
					</div>
				...
				<div class="tab-pane fade" id="pills-resim" role="tabpanel" aria-labelledby="pills-resim-tab">
						...
						<div style="width:480px; height: 44vh;" class="">
							...
							<div class="p-2 text-center bg-white" style="height:30vh; width:100%; box-shadow: 0 0 10px #777;">
								...
								@if(is_file("uploads/resim/hizmet/".$hizmet->resim))
					                <img src="{{ asset('uploads/resim/hizmet/'.$hizmet->resim) }}" style="height:100%; width:auto">
					            @else
					...
					<div class="resim-ekle-wrap my-3 font-weight-bold" style="font-size:16px; margin-top:60px;">
						<i class="fas fa-images"></i> Hizmetin Diğer Resimleri
					</div> 		

						@if( count($hizmet->resimler) == 0 || $hizmet->resimler == null )
							<div class="alert alert-danger d-flex align-items-center">
								<i class="fas fa-info-circle me-2"></i> Hizmete diğer resim eklenmemiş			
							</div>
						...
						@else
							...
							<div class="row" id="sortable">
								@foreach($hizmet->resimler as $resim)
										...
										<div class="col-6 col-md-4 col-lg-3 sortable-wrap sortable" data-index="{{ $resim->id }}" data-sira="{{ $resim->sira }}">
											<div class="eski-resim-wrapper">
												<div class="erw-inside">
													<img src="{{ asset('uploads/resim/hizmet/'.$resim->resim) }}">
												</div>
											</div>
											<div class="text-center my-2">
												<button type="button" class="btn btn-danger hizmet-resim-sil px-2 py-0" data-bs-toggle="tooltip" data-placement="bottom" title="Sil" data-id="{{ $resim->id }}">
				...
				<div class="tab-pane fade" id="pills-video" role="tabpanel" aria-labelledby="pills-video-tab">
					<div class="alert alert-primary d-flex align-items-center mt-5">
						<i class="fas fa-info-circle me-2"></i>
						Hizmet ile ilgili youtube'daki videonuzun linkini veya sitenize video dosyasını bu bölümden ekleyin
					</div>
					...
					<div class="tab-content" id="pills-tabContent">
						<div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="bilgiler">
							<textarea name="video_link" class="form-control" rows="3">{{$hizmet->video_link}}</textarea>
						</div>
						   ...
						   <div class="my-4">
						   		@if(is_file("uploads/video/hizmet/".$hizmet->video))
									...
									<video width="320" height="240" controls>
										<source src="/uploads/video/hizmet/{{$hizmet->video}}" type="video/mp4">	
										<div class="alert alert-danger">
											Tarayıcınız video formatını desteklemiyor
										</div>
									</video>
									<br>
									<button type="button" class="btn btn-danger video-sil my-2" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Sil" data-id="{{ $hizmet->id }}">
										<i class="far fa-trash-alt"></i>
									</button> 
				...
				<div class="tab-pane fade" id="pills-seo" role="tabpanel" aria-labelledby="pills-seo-tab">
					<div class="form-group my-2">
						...
						<textarea name="description" class="form-control" rows="2">{{ $hizmet->description }}</textarea>
					</div>

					<div class="form-group my-2">
						<label for="">Keywords : <small>(Opsiyonel)</small></label>
						<textarea name="keywords" class="form-control" rows="2">{{ $hizmet->keywords }}</textarea>
					</div>
				</div><!-- ./end seo content ------------------------------------------------------------->
...
$(function(){	
	...
	//hizmet resmini sil ----------------------------------------------------------
    $(".hizmet-resim-sil").on("click", function(){                
		...
		Swal.fire({
			...
			if (result.value) {
				...
				var i  = $(".hizmet-resim-sil").index(this);
				var id = $(".hizmet-resim-sil").eq(i).data("id");                

				$.ajax({
					type        : "post",
					url         : "{{ route('hizmet.resim.sil') }}",
					data        : {id:id},
	...
	$("#hizmetDuzenleForm").submit(function(e){
		...
		if(baslik <= 0){
			Swal.fire({
				type: 'error', 
				text: 'Lütfen hizmet başlığını yazınız',
    ...
    $(".video-sil").on("click", function(){  
		...
		Swal.fire({
			...
			if (result.value) {
				...
				$.ajax({
					type        : "post",
					url         : "{{ route('panel.hizmet.video.sil') }}",
					data        : {id:id},
