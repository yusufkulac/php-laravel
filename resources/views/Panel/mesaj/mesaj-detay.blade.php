	...
	<div class="card">
		<div class="card-body">
			<div class="row my-2">
				...
				<div class="col-md-10">: {{ $mesaj->adsoyad }}</div>
			</div>
			<div class="row my-2">
				<div class="col-md-2">Telefon</div>
				<div class="col-md-10">: {{ $mesaj->telefon }}</div>
			</div>
			<div class="row my-2">
				<div class="col-md-2">E-Posta</div>
				<div class="col-md-10">: {{ $mesaj->mail }}</div>
			</div>
			<div class="row my-2">
				<div class="col-md-2">Tarih</div>
				<div class="col-md-10">: 
					{{ date("d-m-Y", strtotime($mesaj->tarih)) }} 
					<i class="lni lni-alarm-clock ms-2"></i>  
					{{ date("H:i", strtotime($mesaj->tarih)) }}
				</div>
			</div>
			<div class="row my-2">
				<div class="col-md-2">İp No</div>
				<div class="col-md-10">: {{ $mesaj->ipno }}</div>
			</div>
			<div class="row my-2">
				<div class="col-md-2">Konu</div>
				<div class="col-md-10">: {{ $mesaj->konu }}</div>
			</div>
			<div class="row my-2 me-2">
				<div class="col-md-12">Mesaj :</div>
				<div class="col-md-12 bg-muted border-muted p-3 mt-2 ms-2">{{ $mesaj->mesaj }}</div>
			</div>

			<button class="btn btn-danger my-4 mesaj-sil me-2" data-id="{{ $mesaj->id }}">
...
<script>
	$(function(){
		...
		$(".mesaj-sil").on("click", function(){ 
			Swal.fire({
					type: "warning",               
					text: "Mesaj silinecek, devam edilsin mi?",          
					...
					$.ajax({
						...
						success : function(result){ 
							...
							if( result.durum == "success"){
								...
								setTimeout(function(){ 
								   window.location.href = "{{ route('panel.mesajlar') }}"
								}, 1500); 
